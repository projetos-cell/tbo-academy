...
// Mapeamento etapa→curso grátis ficará para quando os cursos reais cobrirem
// as áreas do diagnóstico. Sem mock: nenhum curso é "desbloqueado" automaticamente.
  ...
  const weakAreas: WeakArea[] = etapaResults
    ...
    .map((r) => {
      ...
      return {
        ...
        level: r.level,
        insight: "",
        recommendedTrail: trail ? { name: trail.name, desc: trail.desc, modules: trail.modules } : null,
        freeCourse,
      };
    });
...
export function usePreviewSession() {
  ...
  useEffect(() => {
    ...
    if (draftRaw) {
      try {
        ...
        if (hasAnswers) {
          const { weakAreas, totalScore100, totalLevel, unlockedCourseIds } = computeFromDraft(draft);
  ...
  // ─── Actions ────────────────────────────────────────────────
  const trackEvent = useCallback((type: PreviewEventType, metadata: Record<string, unknown> = {}) => {
  ...
  const trackCourseExplored = useCallback(
    (courseId: string) => {
      ...
      const alreadyTracked = s.events.some((e) => e.type === "course_browsed" && e.metadata.courseId === courseId);
      if (!alreadyTracked) {
