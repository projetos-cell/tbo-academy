...
function StepWelcome({ product, onNext }: { product: ProductSlug; onNext: () => void }) {
  return (
    <div className="space-y-6 text-center">
      <div className="inline-flex size-16 items-center justify-center rounded-full bg-[#BAF241]/10 text-3xl text-[#BAF241]">
        🎓
      </div>
      <div>
        <h2 className="text-2xl font-extrabold tracking-tight uppercase">Bem-vindo à TBO Academy</h2>
        <p className="mt-2 text-sm text-zinc-500">
          Você adquiriu o plano{" "}
          <span className="font-semibold text-zinc-800 dark:text-zinc-200">{PRODUCT_LABEL[product]}</span>. Vamos
          configurar sua experiência de aprendizado.
        </p>
      </div>
      <button
        onClick={onNext}
        className="inline-flex items-center gap-2 rounded-lg bg-[#BAF241] px-7 py-3 text-[11px] font-semibold tracking-[1.5px] text-[#000000] uppercase transition-all hover:-translate-y-0.5 hover:shadow-[0_6px_24px_rgba(184,247,36,0.25)]"
      >
        Começar →
      </button>
    </div>
...
function StepDiagnosticResults({ answers, onNext }: { answers: Answers; onNext: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="mb-2 text-[10px] font-medium tracking-[4px] text-[#BAF241] uppercase">Passo 2 de 4</p>
        <h2 className="text-xl font-extrabold tracking-tight uppercase">Seu diagnóstico</h2>
      </div>

      {hasAnswers ? (
        <div className="space-y-3">
          <p className="text-sm text-zinc-500">
            Com base no seu diagnóstico, identificamos as seguintes áreas de desenvolvimento:
          </p>
          {weakAreas.length > 0 ? (
        ...
        <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-5 text-sm text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900">
          Você ainda não completou o diagnóstico. Recomendamos fazer o diagnóstico para receber uma trilha
          personalizada.
        </div>
      )}
      ...
      <div className="flex justify-end">
        <button
          onClick={onNext}
          className="inline-flex items-center gap-2 rounded-lg bg-[#BAF241] px-7 py-3 text-[11px] font-semibold tracking-[1.5px] text-[#000000] uppercase transition-all hover:-translate-y-0.5"
        >
          Próximo →
        </button>
      </div>
...
function StepRecommendedTrail({ answers, onNext }: { answers: Answers; onNext: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="mb-2 text-[10px] font-medium tracking-[4px] text-[#BAF241] uppercase">Passo 3 de 4</p>
        <h2 className="text-xl font-extrabold tracking-tight uppercase">Sua trilha recomendada</h2>
      </div>

      <div className="rounded-xl border border-[#BAF241]/20 bg-[#BAF241]/5 p-5">
        <p className="mb-2 text-[10px] font-semibold tracking-[2px] text-[#BAF241] uppercase">
...
function StepPreferences({
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="mb-2 text-[10px] font-medium tracking-[4px] text-[#BAF241] uppercase">Passo 4 de 4</p>
        <h2 className="text-xl font-extrabold tracking-tight uppercase">Preferências</h2>
      </div>

      <div className="space-y-4">
        {/* Notification frequency */}
        <div>
          <label className="mb-2 block text-[8px] font-semibold tracking-[1.5px] text-zinc-400 uppercase">
          ...
          <div className="flex gap-2">
            {NOTIFICATION_OPTIONS.map((opt) => (
              <button
                key={opt.value}
                onClick={() => onChange({ ...preferences, notificationFrequency: opt.value })}
                className={cn(
                  "rounded-lg border-[1.5px] px-4 py-2 text-[11px] font-semibold transition-all",
                  preferences.notificationFrequency === opt.value
                    ? "border-[#BAF241] bg-[#BAF241]/10 text-zinc-900 dark:text-zinc-100"
                    : "border-zinc-200 text-zinc-500 hover:border-[#BAF241]/40 dark:border-zinc-700",
                )}
              >
                {opt.label}
              </button>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ ...preferences, learningGoal: e.target.value })}
            className="w-full rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-[11px] text-zinc-900 transition-colors outline-none focus:border-[#BAF241] dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
          />
        </div>
      </div>
