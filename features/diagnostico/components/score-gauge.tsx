...
export function ScoreGauge({ score, level, size = 200 }: ScoreGaugeProps) {
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      {/* Score text */}
      <div className="absolute flex flex-col items-center" style={{ bottom: 12 }}>
        <span className="text-4xl font-extrabold tracking-tight" style={{ color: strokeColor }}>
          {animatedScore}
        </span>
        <span className="text-[8px] font-medium tracking-[2px] text-zinc-400 uppercase">de 100 pontos</span>
        <span
          className={cn(
            "mt-1.5 rounded-full px-3 py-0.5 text-[9px] font-bold tracking-[1px] uppercase",
            level.cls === "cego" && "bg-red-500/10 text-red-500",
            level.cls === "miope" && "bg-amber-500/10 text-amber-500",
            level.cls === "enxerga" && "bg-[#BAF241]/10 text-[#BAF241]",
