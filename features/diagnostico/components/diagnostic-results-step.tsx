...
export function DiagnosticResultsStep({ answers, context, onBack, onCTA }: DiagnosticResultsStepProps) {
  const etapaResults = useEtapaScores(answers);

  const totalScore = useMemo(() => etapaResults.reduce((a, r) => a + r.score, 0), [etapaResults]);
  const totalMax = useMemo(() => ETAPAS.reduce((a, e) => a + e.max, 0), []);
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 space-y-7 duration-400">
      {/* ===== MATURITY DASHBOARD ===== */}

      {/* Overall badge */}
      <div className="flex items-center gap-4 rounded-xl bg-[#000000] p-5 text-white">
        <span className="text-3xl">{LEVEL_ICONS[level.idx]}</span>
        <div className="flex-1">
          <h3 className="text-base font-extrabold tracking-tight uppercase">Diagnóstico de Maturidade</h3>
          <p className="mt-0.5 text-[9px] text-zinc-400">Nível geral de conhecimento em marketing imobiliário</p>
        </div>
        <div className="text-right">
          ...
          <p className="text-[8px] tracking-[1px] text-zinc-400 uppercase">{Math.round(totalPct * 100)}% de domínio</p>
          <span
            className={cn(
              "mt-1 inline-block rounded px-2.5 py-0.5 text-[9px] font-semibold tracking-[1px] uppercase",
              colors.bg,
            )}
          >
            {level.name}
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
        <h4 className="mb-4 text-[10px] font-semibold tracking-[2px] text-zinc-400 uppercase">Visão por dimensão</h4>
        <div className="space-y-3">
          {etapaResults.map((r, i) => {
            const c = LEVEL_COLORS[r.level.cls];
            return (
              <div key={i} className="flex items-center gap-3">
                <span className="w-[140px] truncate text-[10px] font-medium text-zinc-600 dark:text-zinc-400">
                  {r.etapa.title}
                </span>
                ...
                <span className={cn("w-10 text-right text-[10px] font-bold", c.text)}>{Math.round(r.pct * 100)}%</span>
                <span className={cn("rounded px-2 py-0.5 text-[7px] font-semibold tracking-[0.5px] uppercase", c.bg)}>
                  {r.level.name}
                </span>
              </div>
            );
          ...
          return (
            ...
            >
              ...
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className={cn("rounded px-2 py-0.5 text-[8px] font-semibold tracking-[1px] uppercase", c.bg)}>
              ...
              <p className="text-[10px] leading-relaxed text-zinc-500">{r.insight}</p>
      ...
      {/* ===== COST OF IGNORANCE ===== */}
      <div className="relative overflow-hidden rounded-xl bg-[#000000] p-6 text-white">
        <div className="absolute inset-x-0 top-0 h-[3px] bg-red-500" />
        <div className="mb-5">
          <h3 className="text-lg font-extrabold tracking-tight uppercase">Custo Estimado da Ignorância</h3>
          <p className="mt-1 text-[10px] text-zinc-400">
            Quanto você perde por não ter repertório de marketing — por empreendimento
          </p>
        </div>
        ...
        <p className="mt-4 border-t border-white/5 pt-4 text-[8px] leading-relaxed text-zinc-500 italic">
          Estimativa conservadora baseada em médias de mercado. O custo real pode ser significativamente maior
          dependendo da praça, concorrência e condições macroeconômicas. O objetivo não é precisão — é dimensionar o que
          a falta de repertório custa.
        </p>
      </div>
      ...
      {sortedTrails.length > 0 && (
        <div>
          <h4 className="mb-4 border-b border-zinc-200 pb-2 text-[10px] font-semibold tracking-[3px] text-zinc-400 uppercase dark:border-zinc-800">
            ...
            {sortedTrails.map((trail, i) => {
              ...
              return (
                <div
                  key={i}
                  className={cn(
                    "flex items-center gap-4 rounded-xl border border-l-4 border-zinc-200 bg-white px-5 py-4 transition-all hover:-translate-y-0.5 hover:shadow-sm dark:border-zinc-800 dark:bg-zinc-900",
                    style.card,
                  )}
                >
                  <span
      ...
      {/* ===== CTA BLOCK ===== */}
      <div className="relative overflow-hidden rounded-xl bg-[#BAF241] p-8 text-center">
        ...
        <p className="relative z-10 mt-3 mb-5 text-[13px] text-[#000000]/50">
          Você ainda terceiriza tudo com as agências
          <br />e ainda não clicou no botão abaixo.
        </p>
        <button
          onClick={onCTA}
          className="relative z-10 inline-flex items-center gap-2 rounded-lg bg-[#000000] px-8 py-4 text-xs font-bold tracking-[1px] text-[#BAF241] uppercase transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(0,0,0,0.3)]"
        >
          Quero enxergar
