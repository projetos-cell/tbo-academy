...
export async function GET() {
  try {
    const supabase = await createUntyped()
    ...
    const moduleIds = [...new Set(progressData.map((r) => r.module_id))]
    ...
    const entries = Object.entries(userModules)
      .map(([userId, modules]) => {
        const completedCourses = Object.entries(userCourseModules[userId] ?? {})
          .filter(([courseId, mods]) => {
